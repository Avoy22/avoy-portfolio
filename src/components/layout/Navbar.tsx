  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? "border-b border-white/[0.06] bg-background/75 backdrop-blur-xl supports-[backdrop-filter]:bg-background/55"
    ...
    >
      ...
      />
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-5 sm:h-[72px] sm:px-8 lg:px-12">
        <Link
          href="/"
          className="group relative flex items-center gap-2.5"
          aria-label={`${site.name} home`}
        >
          <span className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-accent via-accent-2 to-accent-3 text-black shadow-[0_8px_24px_-8px_rgba(109,140,255,0.55),0_0_0_1px_rgba(255,255,255,0.08)_inset] transition-transform duration-300 group-hover:scale-105">
            <Sparkles className="h-4 w-4" strokeWidth={2.5} />
            <span
              aria-hidden
              className="absolute inset-0 rounded-xl bg-gradient-to-tr from-white/30 to-transparent opacity-50"
            />
            <span
              aria-hidden
              className="absolute -inset-0.5 rounded-xl bg-gradient-to-br from-accent/40 via-accent-2/40 to-accent-3/40 opacity-0 blur-md transition-opacity duration-500 group-hover:opacity-100"
            />
          </span>
            ...
            return (
              ...
              >
                {active && (
                  <motion.span
                    layoutId="nav-pill"
                    className="absolute inset-0 -z-10 rounded-full bg-white/[0.06] border border-white/[0.08] shadow-[0_1px_0_rgba(255,255,255,0.04)_inset]"
        ...
        <button
          ...
          aria-expanded={open}
          className="md:hidden inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-white backdrop-blur transition hover:bg-white/[0.08]"
          onClick={() => setOpen((v) => !v)}
        >
                  ...
                  <Link
                    ...
                    className={cn(
                      ...
                      active
                        ...
                        : "text-foreground-muted hover:bg-white/[0.04] hover:text-white"
                    )}
                  >
                    {item.label}
