...
const variants: Record<Variant, string> = {
  ...
  secondary:
    "bg-white/[0.05] text-white border border-white/10 hover:bg-white/[0.09] hover:border-white/20 backdrop-blur shadow-[0_1px_0_rgba(255,255,255,0.04)_inset]",
  ghost:
    "bg-transparent text-foreground-muted hover:text-white hover:bg-white/[0.05]",
  outline:
    "border border-white/15 text-white bg-white/[0.02] hover:border-white/30 hover:bg-white/[0.05] backdrop-blur",
  gradient: cn(
  ...
  const content = (
    <>
      ...
      {(variant === "primary" || variant === "gradient") && (
        ...
        >
          <span className="absolute inset-y-0 left-0 w-[40%] -translate-x-full bg-gradient-to-r from-transparent via-white/35 to-transparent opacity-0 transition-all duration-700 ease-out group-hover/btn:translate-x-[260%] group-hover/btn:opacity-100" />
        </span>
      )}
    </>
  );
